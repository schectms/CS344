...
* encrypt a message received from client and send it back
*/
...
int main(int argc, char* argv[])
{
	...
	while (1)
	{
		...
		case 0:;
			...
			long enc=1; //tracks if enc/dec is trying to connect
			recv(establishedConnectionFD, &enc, sizeof(enc), 0);//receives this info from the client
		
			//if enc is trying to connect
			if (enc == 1)
			{
			
				send(establishedConnectionFD, &enc, sizeof(enc), 0); //send back that it is ok
			}
			//otherwise
			else
			{
			...
			//loops through the length of the plaintext and encrypts it

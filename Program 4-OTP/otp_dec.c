...
int main(int argc, char* argv[])
{
	...
	//checks the content of ciphertext and key file
	if (validChars(ciphertext) == 0 || validChars(key) == 0)
	{
		fprintf(stderr, "%s", "file is invalid");
		exit(1);
	}

	//makes sure that the key is not shorter than ciphertext
	...
	else
	{
		...
		long enc = 0; //tracks if client is enc
		long validConn; // tracks if connection is valid

		// Send the enc variable over
		send(socketFD, &enc, sizeof(enc), 0);

		//receive back validConn variable
		charsWritten = recv(socketFD, &validConn, sizeof(validConn), 0);
		
		//if the connection is valid
		if (validConn == 1)
		{
		// Send the size of ciphertext over
			dataSize = htonl(sizeCT);
			...
			// Send the actual ciphertext
			int bytesSent = 0;
			bytesSent += send(socketFD, ciphertext, sizeCT, 0);
			...
			//send key
			bytesSent = 0;
			bytesSent += send(socketFD, key, sizeCT, 0);
		...
		// Get return message from server
			while (i < (sizeCT - 1))
			{
				i += recv(socketFD, buffer + i, sizeCT - i, 0);
			}
		...
		{
			fprintf(stderr, "%s%d", "error cannot connect to otp_dec_d on port \n", portNumber);
			close(socketFD); // Close the socket
			free(ciphertext);
			free(key);
			exit(2);
